...
from models.network_utils import get_encoding, get_mlp, get_encoding_with_network, se_get_encoding_with_network
...
def contract_to_unisphere(x, radius, contraction_type):
    # print('contraction_type', contraction_type)
...
@models.register('se_volume-density')
class SeVolumeDensity(BaseImplicitGeometry):
    def setup(self):
        self.n_input_dims = self.config.get('n_input_dims', 3)
        self.n_output_dims = self.config.feature_dim
        self.encoding_with_network = se_get_encoding_with_network(self.n_input_dims, self.n_output_dims, self.config.xyz_encoding_config, self.config.mlp_network_config)
                    ...
                    if self.grad_type == 'analytic':
                        ...
                        if self.config.use_appearance_embedding:
                            if self.training and camera_indices is not None:
                                ...
                                appe_embd = self.embedding_appearance(camera_indices.to(self.rank)).repeat(pts_embd_d.size()[0], 1)
                            else:
